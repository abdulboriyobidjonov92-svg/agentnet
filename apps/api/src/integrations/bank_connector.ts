 ...
 * Bitta umumiy BankConnector interfeysi orqali Payme, Click va Plaid
 * adapterlarini birlashtiradi. Har bir tranzaksiya riba/foiz belgisi bilan teglanadi.
 */
...
// ----------------------------------------------------------------
// Riba/foiz aniqlash heuristikasi
// ----------------------------------------------------------------
...
export class ClickAdapter implements BankConnector {
  ...
  constructor(
    ...
    this.client.interceptors.request.use((config) => {
      config.headers['Auth'] = this._buildAuthHeader();
      return config;
    });
  }
  ...
  private _buildAuthHeader(): string {
    ...
    const digest = crypto
      ...
      .digest('hex');
    return `${this.merchantId}:${digest}:${timestamp}`;
  }
