...
`AppModule` wires ~24 feature modules: `auth`, `prisma`, `crypto`, `agents`, `conversations`, `users`, `marketplace`, `telegram`, `twin`, `goals`, `intelligence`, `agentos`, `automation`, `connectors`, `retail`, `operations`, `govtech`, `trade`, `usage`, `billing`, `templates`, `feedback`, `share`, `referral`, `briefing`. Global: `ThrottlerModule` (100 req/min per IP, in-memory), `ScheduleModule` (cron — e.g. daily goal advancement/briefing), `APP_GUARD → ThrottlerGuard`, `APP_FILTER → AllExceptionsFilter`.
