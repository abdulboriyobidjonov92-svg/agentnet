...
#    (sandboxing tamoyili: agent o'zboshimchalik bilan tool ishlatolmaydi)
# ------------------------------------------------------------------
...
# Misol tool implementatsiyalari (har biri haqiqiy integratsiyaga ulanadi)
...
class AgentEngine:
    ...
    async def _reason_node(self, state: AgentState) -> AgentState:
        """LLM'ga system prompt + tarix + ruxsat etilgan tool'lar ro'yxatini beradi."""
        system = {
            "role": "system",
            "content": (
        ...
        # Claude'ga chaqiruv (tool-use bilan) -- soddalashtirilgan ko'rinish:
    ...
    async def _execute_tools_node(self, state: AgentState) -> AgentState:
        ...
        for call in state.get("pending_tool_calls", []):
            ...
            if tool_id not in allowed_ids:
                raise PermissionError(f"Agent '{self.definition.name}' uchun ruxsatsiz tool: {tool_id}")
    ...
    async def _halal_check_output(self, state: AgentState) -> AgentState:
        """Chiquvchi javobni ham tekshiradi (masalan, riba targ'iboti bo'lmasin)."""
        return state
...
# ------------------------------------------------------------------
# 5. No-code JSON'dan to'g'ridan-to'g'ri agent yaratish (builder UI uchun)
# ------------------------------------------------------------------


def create_agent_from_nocode_json(payload: dict[str, Any]) -> AgentEngine:
    """
    Frontend React Flow canvas'dan kelgan JSON shu formatda bo'lishi kutiladi:

    {
      "agent_id": "uuid",
      "name": "Namoz vaqti yordamchisi",
      "system_prompt": "Sen ...",
      "tools": [{"tool_id": "islam.prayer_times", "config": {"city": "Tashkent"}}],
